...
use crate::{Entity, GC, Transform, V2, conv_vec, deassemble_iso, get_angle, get_texture_origin, pi_to_pi2_range, polar_to_cartesian};

const TARGET_POS_DIST_MARGIN: f32 = 50.0;
...
impl ShipAttributes {
    pub fn caravel() -> ShipAttributes {
        ShipAttributes {
            movement_speed: 4.5, turn_rate: 1.0
        }
    }
}
...
impl Ship {
    ...
    fn move_to_target_pos(&mut self) {
        if let Some(target_pos) = self.target_pos {
            ...
            let wind = game_ref.world.wind;
            let rb = game_ref.physics.get_rb_mut(self.transform.handle.0);
            let (pos, rot) = deassemble_iso(rb.position());
            ...
            facing_dir += wind;
            facing_dir *= 10000.0 * self.attr.movement_speed;
            ...
            let mut applied_torque = self.attr.turn_rate * 3000000.0;
...
impl Entity<Ship> for Ship {
    fn init(ctx: &mut Context, game: GC) -> tetra::Result<Ship> {
...
impl State for Ship {
    ...
    fn draw(&mut self, ctx: &mut Context) -> tetra::Result {
        self.ship_texture.draw(ctx, self.transform.get_draw_params());
        Ok(())
    }
}
